...
fn apply_reductions(cx: &LateContext<'_>, nbits: u64, expr: &Expr<'_>, signed: bool) -> u64 {
    match expr.kind {
        ...
        ExprKind::MethodCall(method, _, [left, right], _) => {
            ...
            let max_bits = if method.ident.as_str() == "max" {
        ...
        ExprKind::MethodCall(method, _, [value, lo, hi], _) => {
            let reduced_bits = apply_reductions(cx, nbits, value, signed);
            if method.ident.as_str() == "clamp" {
                //TODO: make this a diagnostic item
                if let (Some(lo_bits), Some(hi_bits)) = (get_constant_bits(cx, lo), get_constant_bits(cx, hi)) {
                    reduced_bits.min(lo_bits.max(hi_bits))
                } else {
                    reduced_bits
                }
            } else {
